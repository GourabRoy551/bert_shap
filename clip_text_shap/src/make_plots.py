"""Numerically annotated PNG and PDF figures for the CLIP text experiment."""
...
def _annotate_cells(
    axis: plt.Axes, values: np.ndarray, fmt: str = "+.4f", limit: float | None = None
) -> None:
    colour_limit = limit or _symmetric_limit(values)
    for row in range(values.shape[0]):
        for column in range(values.shape[1]):
            ...
            axis.text(
                ...
                va="center",
                fontsize=8.2,
                color="white" if abs(value) > 0.58 * colour_limit else "black",
            )
...
def _draw_shap_matrix(
    ...
    axis.set_xlabel("Explained CLIP text-prototype similarity")
    axis.set_title(title, pad=10)
    _annotate_cells(axis, matrix, limit=limit)
    return image
...
def save_dual_word_bar(
    ...
    limit = 1.22 * _symmetric_limit(matrix)
    ...
    for class_index, axis in enumerate(axes):
        ...
        bars = axis.barh(positions, class_values, color=colours, alpha=0.88)
        axis.axvline(0.0, color="black", linewidth=1.0)
        axis.set_xlim(-limit, limit)
        axis.set_yticks(positions, labels=words)
        axis.grid(axis="x", alpha=0.22)
        axis.set_xlabel(f"SHAP value for {CLASS_NAMES[class_index]} similarity")
        axis.set_title(
            f"Target: {CLASS_NAMES[class_index]}\n"
            f"score={float(scores[class_index]):+.4f} | "
    ...
    figure.suptitle(
        f"{sentence_id}: CLIP text dual-class Partition SHAP\n"
    ...
    figure.text(
        0.5,
        0.006,
        "Red increases the named class similarity; blue decreases it. "
...
def save_class_value_matrix(
    ...
    figure, axis = plt.subplots(figsize=(8.8, height))
...
def summary_values(
    ...
    rows = [
        "Cosine similarity",
        "Softmax share (uncalibrated)",
        "SHAP base value",
        "Sum of SHAP values",
        "Reconstructed similarity",
        "Additivity residual",
        "Mean absolute SHAP",
        "Total absolute SHAP",
    ]
    values = np.vstack(
        [
            ...
            np.asarray(softmax_shares, dtype=float),
...
def _draw_summary_matrix(
    axis: plt.Axes, row_labels: Sequence[str], raw_values: np.ndarray, title: str
) -> None:
    ...
    for row in range(raw_values.shape[0]):
        for column in range(2):
            ...
            axis.text(
                column,
                row,
                format(float(raw_values[row, column]), fmt),
                ha="center",
                va="center",
                fontsize=8.5,
...
def save_sentence_summary_matrix(
    ...
    rows, raw = summary_values(
        scores, softmax_shares, base_values, shap_values, reconstructed, residuals
    )
    figure, axis = plt.subplots(figsize=(8.8, 6.8))
    _draw_summary_matrix(axis, rows, raw, f"{sentence_id}: sentence-summary matrix")
    figure.suptitle(
        f"gold={gold_label} | prediction={prediction}\n"
...
def save_sentence_overview(
    ...
    height = max(8.0, 0.34 * max(len(words), len(rows)) + 4.2)
    figure, axes = plt.subplots(
        1, 2, figsize=(14, height), gridspec_kw={"width_ratios": [1.15, 1.0]}
    )
    image = _draw_shap_matrix(axes[0], words, word_values, "Word-level SHAP values")
    figure.colorbar(image, ax=axes[0], fraction=0.045, pad=0.025).set_label(
        "SHAP value"
    )
    _draw_summary_matrix(axes[1], rows, raw, "Sentence-summary values")
    figure.suptitle(
        f"{sentence_id}: CLIP text dual-class SHAP overview\n"
        f"gold={gold_label} | prediction={prediction}\n"
...
def save_all_sentence_summary_matrix(
    ...
    values = np.asarray(
        [
            [
                row["negative_score"],
                row["positive_score"],
                row["negative_softmax_share"],
                row["positive_softmax_share"],
    ...
    scales = np.max(np.abs(values), axis=0, keepdims=True)
    scales[scales < 1e-12] = 1.0
    ...
    axis.set_title("All-sentence CLIP text dual-class summary matrix", pad=12)
    for row in range(values.shape[0]):
        for column in range(values.shape[1]):
            fmt = ".1e" if column >= 6 else ".4f"
            axis.text(
                column,
                row,
...
def save_sentence_class_magnitude_matrix(
    ...
    axis.set_xticks([0, 1], labels=CLASS_NAMES)
    axis.set_yticks(np.arange(len(labels)), labels=labels)
    axis.set_title("Mean absolute word SHAP by sentence and class", pad=11)
    for row in range(values.shape[0]):
        for column in range(2):
            axis.text(
                column,
                row,
                f"{values[row, column]:.4f}",
                ha="center",
                va="center",
                fontsize=8.2,
...
def save_global_word_class_matrix(
    output_stem: Path, word_rows: list[dict[str, Any]], top_n: int = 20
) -> None:
    ...
    ranked = [
        (
            word,
            [float(np.mean(values[name])) if values[name] else 0.0 for name in CLASS_NAMES],
        )
...
def save_additivity_residual_matrix(
    ...
    _annotate_cells(axis, values, fmt=".2e", limit=limit)
    figure.colorbar(image, ax=axis, fraction=0.045, pad=0.025).set_label(
        "CLIP score minus SHAP reconstruction"
    )
    figure.tight_layout()
    save_figure(figure, output_stem)
...
def save_class_comparison_bars(
    output_stem: Path, word_rows: list[dict[str, Any]], top_n: int = 10
) -> None:
    ...
    means = {key: float(np.mean(values)) for key, values in grouped.items() if values}
    figure, axes = plt.subplots(2, 2, figsize=(15, 11))
    for class_index, class_name in enumerate(CLASS_NAMES):
        class_items = [
            (word, value) for (name, word), value in means.items() if name == class_name
        ]
        increasing = sorted(
        ...
        decreasing = sorted(
            (item for item in class_items if item[1] < 0), key=lambda item: item[1]
        )[:top_n]
        for column, (items, direction) in enumerate(
            ...
            colour = INCREASE_COLOR if column == 0 else DECREASE_COLOR
            bars = axis.barh(np.arange(len(words)), values, color=colour, alpha=0.88)
            axis.set_yticks(np.arange(len(words)), labels=words)
            axis.axvline(0.0, color="black", linewidth=0.9)
            axis.margins(x=0.16)
            axis.grid(axis="x", alpha=0.2)
            axis.set_title(f"{direction} {class_name} similarity")
    ...
    figure.suptitle(
        "Words with the strongest mean class-specific CLIP SHAP contributions",
        fontsize=15,
    )
